...
function ReturnpostlistPage(postlistmetainfo: {
  title: string;
  postlist: {
    filters: { category: string | undefined; tag: string | undefined } | undefined;
  };
}) {
  let arrayofallposts;
  let lastdatestamp = 1;
  ...
  if (
    ...
    debuglog("Returning a filtered post list.");
    for (const i in arrayofallposts) {
      const post = arrayofallposts[i];

      if (postlistmetainfo.postlist.filters.tag !== undefined) {
        if (post.tags === undefined) continue;
        debuglog(`Compare: '${postlistmetainfo.postlist.filters.tag}' vs '${JSON.stringify(post.tags)}'`);
        if (!post.tags.includes(postlistmetainfo.postlist.filters.tag)) continue;
        debuglog("Match.")
      }



      if (postlistmetainfo.postlist.filters.category !== undefined) {
        debuglog(`Compare: '${postlistmetainfo.postlist.filters.category}' vs '${post.category}'`);
        if (post.category !== postlistmetainfo.postlist.filters.category) continue;
        debuglog("Match.")
      }
      const addition = `
			<tr><td class="post-date"><span class="unparsedtimestamp post-date">${post.dates.published
        }</span></td><td><a href="/p/${post.id}"><span class="post-title">${md.render(
          post.title,
        )}</span></a>
			</td><td class="post-category"><a href="/c/${post.category}">${post.category
...
async function ReturnPage(id: string, currenturl: string) {
  // Get page meta info
  ...
  if (pagemeta.postlist !== undefined) {
    const postlistp = ReturnpostlistPage(pagemeta);
    if (typeof postlistp === "string") pagecontent = postlistp; else pagecontent = "";
...
async function BlogPagesRespond(filters: [filtertype: string, filter: string], req, res) {
  debuglog(
  ...
  try {
    let vtag;
    let vcat;
    if (filters[0].toLowerCase() === "tag") vtag = filters[1]; else vtag = undefined;
    if (filters[0].toLowerCase() === "category") vcat = filters[1]; else vcat = undefined;
    const virtualpostobject = {
      title: (`${filters[0].charAt(0).toUpperCase() + filters[0].slice(1)}: ${filters[1]}`), postlist: {
        filters:
          { category: vcat, tag: vtag },
      }
    };

    const PostTableHTML = ReturnpostlistPage(virtualpostobject);
    // Get page meta info
    let pagemode: string;
    // Get info about what template to use
    if (
      returnpagemeta("root").mode === undefined ||
      returnpagemeta("root").mode == null ||
      returnpagemeta("root").mode === ""
    )
      pagemode = "default";
    else pagemode = returnpagemeta("root").mode;
    let pagetype: string;
    if (returnpagemeta("root").type === "post") pagetype = "post";
    else pagetype = "page";
    const handlebarsfile = modes[pagemode].handlebar[pagetype];
    // Get actual page content
    let rawpagecontent: string;
    let pagecontent = PostTableHTML;
    // Prepare menu links
    // debuglog("Menu links 1");
    let menu1links = "";
    let menu1_links;
    if (
      returnpagemeta("root").menu1linkoverride === undefined ||
      returnpagemeta("root").menu1linkoverride == null ||
      returnpagemeta("root").menu1linkoverride.length === 0
    )
      menu1_links = modes[pagemode].menu1links;
    else menu1_links = returnpagemeta("root").menu1linksoverride;
    if (menu1_links === undefined || menu1_links == null)
      menu1_links = modes[pagemode].menulinks;
    if (menu1_links !== undefined && menu1_links !== null) {
      menu1_links.forEach((link) => {
        if (link.href === req.originalUrl)
          menu1links = `${menu1links}<a href="${link.href}" class="active">${link.name}</a>`;
        else menu1links = `${menu1links}<a href="${link.href}">${link.name}</a>`;
      });
    }
    ...
    let menu2_links;
    if (
      returnpagemeta("root").menu2linkoverride === undefined ||
      returnpagemeta("root").menu2linkoverride == null ||
      returnpagemeta("root").menu2linkoverride.length === 0
    )
      menu2_links = modes[pagemode].menu2links;
    else menu2_links = returnpagemeta("root").menu2linksoverride;
    if (menu2_links !== undefined && menu2_links !== null) {
      menu2_links.forEach((link) => {
        if (link.href === req.originalUrl)
          menu2links = `${menu2links}<a href="${link.href}" class="active">${link.name}</a>`;
	...
	<script src="/jquery/jquery.min.js"></script>
	<title>${virtualpostobject.title} ﹘ ${modes[pagemode].sitename}</title>
	<script>
		const pagemetainfo = JSON.parse(\`${JSON.stringify(virtualpostobject)}\`);
	</script>
	`;
    // Run body modifier plugins.
    cynthiabase.modifyBodyHTML.forEach((modifier) => {
...
-->
    ...
    res.send(`<!DOCTYPE html>${page}<script>${fs.readFileSync(
      path.join(__dirname, "../src/client.js"),
      {
        encoding: "utf8",
        flag: "r",
...
app.get("/c/*", async (req, res) => {
  const category: string = decodeURI(req.originalUrl.replace("/c/", "").replace(/\/$/, ""));
  if (category === "") {
    res.redirect("/");
    return;
  }
...
app.get("/t/*", async (req, res) => {
  const category: string = decodeURI(req.originalUrl.replace("/t/", "").replace(/\/$/, ""));
  if (category === "") {
    res.redirect("/");
    return;
  }
  BlogPagesRespond(["Tag", category], req, res);
});
